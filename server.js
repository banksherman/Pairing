...
// Improved QR endpoint: waits until QR is generated or timeout
app.get("/qr", async (req, res) => {
  ...
  s.sock.ev.on("connection.update", (update) => {
    if (update.qr && !responded) {
      ...
      return res.json({
        ok: true,
        qr: update.qr, // Baileys provides base64 PNG string
        session: sessionId
      });
    }
  });
  ...
  setTimeout(() => {
    if (!responded) {
      res.json({ ok: false, message: "QR not generated. Try again." });
    }
  }, 30000);
});
